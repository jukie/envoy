...
class LeastRequestZoneAwareLoadBalancerTest : public LoadBalancerTestBase {
public:
  void init() {
    local_priority_set_ = std::make_shared<PrioritySetImpl>();
    local_priority_set_->getOrCreateHostSet(0);
    lb_ = std::make_shared<LeastRequestLoadBalancer>(priority_set_, local_priority_set_.get(),
                                                     stats_, runtime_, random_, 50, config_,
                                                     simTime());
  }
  ...
  std::shared_ptr<LeastRequestLoadBalancer> lb_;
  HostsPerLocalityConstSharedPtr empty_locality_;
  HostVector empty_host_vector_;
};

TEST_P(LeastRequestZoneAwareLoadBalancerTest, OrcaLocalityPrefersLocalCapacity) {
  ...
  auto* zone_config = config_.mutable_locality_lb_config()
...
TEST_P(LeastRequestZoneAwareLoadBalancerTest, OrcaLocalityRoutesResidualWhenLocalSaturated) {
  envoy::config::core::v3::Locality zone_a;
  zone_a.set_zone("zone_a");
  envoy::config::core::v3::Locality zone_b;
  zone_b.set_zone("zone_b");
...
INSTANTIATE_TEST_SUITE_P(PrimaryOrFailoverAndLegacyOrNew, LeastRequestZoneAwareLoadBalancerTest,
                         ::testing::Values(LoadBalancerTestParam{true},
                                           LoadBalancerTestParam{false}));

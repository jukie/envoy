...
TEST_F(OrcaOobManagerWireTest, ReportWithoutLbPolicyDataIsNoOp) {
  // A host with no ORCA-interested data drops the report (no recipient), matching the in-band
  // path: reports_received still bumps, but report_errors does not.
  auto manager = makeManager();
  ASSERT_OK(manager->initialize());

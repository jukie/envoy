...
public:
  ClientSideWeightedRoundRobinLoadBalancer(OptRef<const Upstream::LoadBalancerConfig> lb_config,
                                           ...
                                           TimeSource& time_source);
  ~ClientSideWeightedRoundRobinLoadBalancer() override;
...
private:
  ...
  // OOB load reporting config and manager.
  bool enable_oob_load_report_{false};
  std::chrono::milliseconds oob_reporting_period_{10000};
  std::chrono::milliseconds oob_initial_jitter_{10000};
  std::unique_ptr<Orca::OrcaOobManager> oob_manager_;

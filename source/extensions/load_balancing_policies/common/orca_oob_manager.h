 ...
 * report is delivered to the host's attached `HostLbPolicyData` via `onOrcaLoadReport` — the same
 * entry point as the in-band request path — so any policy that attaches ORCA-interested host data
 * receives OOB reports with no OOB-specific glue.
 */
...
public:
  ...
  // Init order constraint: whatever per-host data the report recipients expect must be attached
  // before sessions decode their first report (for CSWRR, call OrcaWeightManager::initialize()
  // first). A report arriving before any interested data is attached is simply dropped (no
  // recipient), matching the in-band path — not counted as an error.
